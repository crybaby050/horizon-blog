...
<div class="adm-page">
  ...
  <div class="adm-filters fade-up">
    ...
    <div class="adm-filter-chips">
      <a href="<?= path('admin','signalements',['type'=>$type,'q'=>$search]) ?>"
         class="adm-chip <?= empty($statut)?'active':'' ?>">Tous</a>
      <a href="<?= path('admin','signalements',['statut'=>'Non traiter','type'=>$type,'q'=>$search]) ?>"
         class="adm-chip <?= $statut==='Non traiter'?'active':'' ?>">Non traités</a>
      <a href="<?= path('admin','signalements',['statut'=>'Traiter','type'=>$type,'q'=>$search]) ?>"
         class="adm-chip <?= $statut==='Traiter'?'active':'' ?>">Traités</a>
    </div>

    <!-- Filtre type -->
    <div class="adm-filter-chips" style="margin-top:8px">
      <a href="<?= path('admin','signalements',['statut'=>$statut,'q'=>$search]) ?>"
         class="adm-chip <?= empty($type)?'active':'' ?>">Tous types</a>
      <a href="<?= path('admin','signalements',['statut'=>$statut,'type'=>'article','q'=>$search]) ?>"
         class="adm-chip <?= $type==='article'?'active':'' ?>">
        <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" width="11" height="11">
          <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
        </svg>
        Articles
      </a>
      <a href="<?= path('admin','signalements',['statut'=>$statut,'type'=>'commentaire','q'=>$search]) ?>"
         class="adm-chip <?= $type==='commentaire'?'active':'' ?>">
        <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" width="11" height="11">
  ...
  <div class="adm-signal-grid fade-up" style="transition-delay:.05s">
    <?php foreach ($signalements as $sig):
      ...
      $isArticle     = empty($sig['commentaire_id']) && !empty($sig['article_id']);
      $type_label    = $isArticle ? 'Article' : 'Commentaire';
    ?>
    <div class="adm-signal-card <?= $isTraite ? 'adm-signal-card-done' : '' ?>">
      ...
      <!-- Contenu signalé -->
      <?php if ($isArticle && !empty($sig['article_libelle'])): ?>
        <?php
          $titreArticle = $sig['article_libelle'];
          $titreArticle = strlen($titreArticle) > 60
              ? substr($titreArticle, 0, 60) . '…'
              : $titreArticle;
        ?>
        <div class="adm-signal-card-ref">
          <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" width="12" height="12">
            <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/>
            <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
          </svg>
          <strong>Article :</strong> <?= htmlspecialchars($titreArticle) ?>
        </div>
      <?php endif; ?>

      <?php if (!$isArticle && !empty($sig['commentaire_contenu'])): ?>
        <?php
          $contenuCom = $sig['commentaire_contenu'];
          $contenuCom = strlen($contenuCom) > 80
              ? substr($contenuCom, 0, 80) . '…'
              : $contenuCom;
        ?>
        <div class="adm-signal-card-ref" style="font-style:italic">
          <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" width="12" height="12">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
          </svg>
          <strong>Commentaire :</strong> «&nbsp;<?= htmlspecialchars($contenuCom) ?>&nbsp;»
        </div>
      <?php endif; ?>
      ...
      <div class="adm-signal-card-actions">
        ...
        <?php if (!$isTraite): ?>
          ...
          <?php if ($isArticle): ?>
            <!-- Action 1 : invalider l'article + traiter -->
            <form method="POST" action="<?= path('admin','signalements') ?>" style="display:inline">
              <input type="hidden" name="post_action"    value="invalider_article"/>
              <input type="hidden" name="signalement_id" value="<?= $sig['id'] ?>"/>
              <input type="hidden" name="article_id"     value="<?= $sig['article_id'] ?>"/>
              <button type="submit" class="adm-signal-btn adm-signal-btn-del"
                      onclick="return confirm('Invalider cet article et marquer le signalement comme traité ?')">
          ...
          <?php else: ?>
            <!-- Action 1 : supprimer le commentaire + traiter -->
            <form method="POST" action="<?= path('admin','signalements') ?>" style="display:inline">
              <input type="hidden" name="post_action"      value="supprimer_commentaire"/>
              <input type="hidden" name="signalement_id"   value="<?= $sig['id'] ?>"/>
              <input type="hidden" name="commentaire_id"   value="<?= $sig['commentaire_id'] ?>"/>
              <button type="submit" class="adm-signal-btn adm-signal-btn-del"
                      onclick="return confirm('Supprimer ce commentaire et marquer le signalement comme traité ?')">
                <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" width="13" height="13">
        ...
        <form method="POST" action="<?= path('admin','signalements') ?>" style="display:inline">
          ...
          <button type="submit" class="adm-signal-btn adm-signal-btn-ignore"
                  onclick="return confirm('Ignorer et supprimer ce signalement ?')">
            <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" width="13" height="13">
  ...
  <div class="adm-pagination fade-up">
    <?php if ($page > 1): ?>
      <a href="<?= path('admin','signalements',['page'=>$page-1,'statut'=>$statut,'q'=>$search,'type'=>$type]) ?>" class="adm-page-btn">‹</a>
    <?php endif; ?>
    <?php for ($p = max(1,$page-2); $p <= min($totalPages,$page+2); $p++): ?>
      <a href="<?= path('admin','signalements',['page'=>$p,'statut'=>$statut,'q'=>$search,'type'=>$type]) ?>"
