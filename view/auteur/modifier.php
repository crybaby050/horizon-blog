...
<div class="au-page">
  ...
  <div class="au-page-header fade-up">
    <div>
      <h1 class="au-page-title">Modifier l'article</h1>
      <p class="au-page-sub">Les modifications remettront l'article en attente de validation.</p>
    </div>
    <a href="<?= path('auteur','detail',['id'=>$article['id']]) ?>" class="au-btn-ghost">
  ...
  <?php if (!empty($errors['global'])): ?>
    <div class="au-alert au-alert-err fade-up"><?= htmlspecialchars($errors['global']) ?></div>
  <?php endif; ?>

  <form method="POST"
        action="<?= path('auteur','modifier',['id'=>$article['id']]) ?>"
    ...
    <input type="hidden" name="post_action" value="edit_article"/>
    ...
    <div class="au-form-grid">
      ...
      <div class="au-form-main">
        ...
        <div class="au-field <?= !empty($errors['titre']) ? 'au-field-err' : '' ?>">
          <label for="af-titre">Titre <span class="au-req">*</span></label>
          <input type="text" id="af-titre" name="titre"
                 value="<?= htmlspecialchars($article['libelle']) ?>"
        ...
        <div class="au-field <?= !empty($errors['description']) ? 'au-field-err' : '' ?>">
          <label for="af-desc">Description <span class="au-req">*</span></label>
          <textarea id="af-desc" name="description" rows="3"
                    placeholder="Description…" required><?= htmlspecialchars($article['description']) ?></textarea>
        ...
        <div class="au-field <?= !empty($errors['contenu']) ? 'au-field-err' : '' ?>">
          <label for="af-contenu">Contenu <span class="au-req">*</span></label>
      ...
      <div class="au-form-sidebar">
        ...
        <div class="au-form-card">
          ...
          <p class="au-form-card-hint">Cochez pour supprimer une image.</p>
          <div class="au-existing-images">
            <?php foreach ($imagesArticle as $img): ?>
            <div class="au-existing-img">
              <img src="<?= htmlspecialchars($img['url']) ?>" alt="<?= htmlspecialchars($img['legende'] ?? '') ?>"/>
              <label class="au-img-del-label">
                <input type="checkbox" name="suppr_images[]" value="<?= $img['id'] ?>"/>
                <span class="au-img-del-overlay">
                  <svg fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke-linecap="round" width="18" height="18" stroke="#fff">
        ...
        <div class="au-form-card <?= !empty($errors['images']) ? 'au-field-err' : '' ?>">
          ...
          <div class="au-upload-zone" id="uploadZone"
            ...
            <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke-linecap="round" width="32" height="32" stroke="#9ca3af">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
              <polyline points="17 8 12 3 7 8"/>
              <line x1="12" y1="3" x2="12" y2="15"/>
            </svg>
            <p>Cliquez ou glissez vos images<br/><span>JPG, PNG, WEBP — max 5 Mo</span></p>
          </div>
        ...
        <div class="au-form-card <?= !empty($errors['categories']) ? 'au-field-err' : '' ?>">
          ...
          <?php
            $selectedCatIds = array_column($categoriesArticle, 'id');
          ?>
          <div class="au-categ-list">
            <?php foreach ($categories as $cat):
              $checked = in_array($cat['id'], $selectedCatIds);
            ?>
        ...
        <div class="au-form-card au-form-submit-card">
          ...
          <p class="au-form-note">L'article repassera en attente de validation.</p>
        </div>
